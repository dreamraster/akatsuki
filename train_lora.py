...
#!/usr/bin/env python3
"""
Unified LoRA fine-tuning for Qwen Models

This script provides comprehensive LoRA (Low-Rank Adaptation) fine-tuning
with automatic hardware detection. It prioritizes:
1. NVIDIA CUDA (GPU)
2. AMD ROCm / HIP (GPU)
3. CPU (Fallback)

Usage:
    # Automatic hardware detection (prioritizes GPU)
    python train_lora.py --model_name "Qwen/Qwen3-0.6B" --data_path "TeichAI/claude-4.5-opus-high-reasoning-250x"

    # Force CPU training explicitly
    python train_lora.py --model_name "Qwen/Qwen3-0.6B" --data_path "TeichAI/claude-4.5-opus-high-reasoning-250x" --cpu

    # With custom settings and 16-bit precision without 4-bit (ideal for ROCm stability)
    python train_lora.py \
        --model_name "Qwen/Qwen3.5-2B" \
        --data_path "dataset1, dataset2" \
        --num_train_epochs 3 \
        --no_4bit
"""
...
def train_lora(
    ...
    output_dir: str = "./checkpoints/qwen-lora",
    ...
    """
    Train Qwen models using LoRA with automatic hardware fallback.
    """
    logger.info("=" * 50)
    logger.info("Unified Qwen LoRA Fine-tuning")
    ...
    if force_cpu:
        logger.warning("User explicitly requested CPU training. Bypassing GPU checks.")
        ...
        if hasattr(torch.version, 'hip') and torch.version.hip is not None:
            ...
            hardware_name = "ROCm / HIP (GPU)"
        else:
            logger.info(f"CUDA version: {torch.version.cuda}")
            logger.info(f"cuDNN version: {torch.backends.cudnn.version()}")
            hardware_name = "CUDA (GPU)"
    else:
        logger.warning("GPU not available - falling back to CPU (This will be very slow!)")
    ...
    try:
        ...
        logger.info("Make sure you have access to the model on Hugging Face")
        return None
    ...
    if not use_gpu and load_in_4bit:
        logger.warning("CPU training detected. Disabling 4-bit quantization automatically as it causes crashes on CPU.")
        load_in_4bit = False

    # Load model
    logger.info(f"4/6 Loading model into memory...")
    try:
        # Use bfloat16 for modern GPUs, float32 for CPU
        if use_gpu and torch.cuda.is_bf16_supported() and use_mixed_precision:
            torch_dtype = torch.bfloat16
        elif use_gpu and use_mixed_precision:
        ...
        if load_in_4bit:
            try:
                ...
                if hasattr(torch.version, 'hip') and torch.version.hip is not None:
                    ...
                    logger.warning("Example: python train_lora.py --no_4bit")
                    logger.warning("!"*60 + "\n")
                return None
            ...
            try:
                model = AutoModelForCausalLM.from_pretrained(
                    ...
                    quantization_config=BitsAndBytesConfig(**bnb_config) if load_in_4bit else None,
                )
            ...
            # Full precision, fp16, bf16 or full CPU loading
            model = AutoModelForCausalLM.from_pretrained(
                model_name,
                trust_remote_code=True,
                torch_dtype=torch_dtype,
    ...
    # Hardware optimized Training arguments
    ...
    training_args = TrainingArguments(
        ...
        fp16=use_gpu and not torch.cuda.is_bf16_supported() if use_mixed_precision else False,
        bf16=use_gpu and torch.cuda.is_bf16_supported() if use_mixed_precision else False,
    )
    ...
    # Start training
    logger.info("=" * 50)
    logger.info(f"Starting {hardware_name} LoRA fine-tuning...")
    logger.info(f"Internal assigned device namespace: {trainer.args.device}")
    logger.info("=" * 50)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Unified Hardware-Agnostic Qwen LoRA Fine-tuning"
    )
    ...
    parser.add_argument("--data_path", type=str, default="TeichAI/claude-4.5-opus-high-reasoning-250x",
                        help="Dataset name or path (comma separated for multiple)")
    ...
    parser.add_argument("--cpu", action="store_true",
                        help="Explicitly force CPU training (bypasses automatic GPU detection)")
    ...
    parser.add_argument("--no_4bit", dest="use_4bit", action="store_false", default=True,
                        help="Disable 4-bit quantization (Required if running ROCm without custom BNB binaries)")
